...
func (s *Service) CreateFromAction(ctx context.Context, action *domainAction.Action) (*notification.Notification, error) {
	// Idempotency: avoid duplicate notifications for the same action
	existingByAction, err := s.notificationRepo.GetByActionID(ctx, action.ActionID)
	if err != nil {
		return nil, fmt.Errorf("failed to check existing notifications: %w", err)
	}
	if len(existingByAction) > 0 {
		existing := existingByAction[len(existingByAction)-1]
		s.logger.Info().
			Str("notification_id", existing.NotificationID.String()).
			Str("action_id", action.ActionID.String()).
			Msg("notification already exists for action")
		return existing, nil
	}
	...
	// Build dedupe key (optional, scoped by channel/target)
	var dedupeKey *string
	if action.DedupeKey != nil && *action.DedupeKey != "" {
		key := *action.DedupeKey
		if channel != "" {
			key += "|channel:" + string(channel)
		}
		if actionCfg.UserID != nil {
			key += "|user:" + *actionCfg.UserID
		}
		if actionCfg.Group != nil {
			key += "|group:" + *actionCfg.Group
		}
		dedupeKey = &key
		cooldown := dedupeWindow(action)
		if cooldown > 0 {
			since := time.Now().UTC().Add(-cooldown)
			existing, err := s.notificationRepo.FindByDedupeKey(ctx, key, since)
			if err != nil {
				return nil, fmt.Errorf("failed to check notification dedupe: %w", err)
			}
			if existing != nil && !existing.IsExpired() {
				s.logger.Info().
					Str("notification_id", existing.NotificationID.String()).
					Str("dedupe_key", key).
					Msg("notification deduped")
				return existing, nil
			}
		}
	}
...
func dedupeWindow(action *domainAction.Action) time.Duration {
	...
	window := action.CooldownUntil.Sub(action.CreatedAt)
	if window < 0 {
		return 0
	}
	return window
...
// CreateEscalation creates a notification for escalation scenarios.
func (s *Service) CreateEscalation(
	ctx context.Context,
	actionID uuid.UUID,
	...
	if body == "" {
		body = "A workflow step requires attention."
	}

	n := notification.NewNotification(actionID, channel, notification.PriorityHigh, title, body, []byte("{}"))
	n.SetTarget(targetUser, targetGroup)
	if traceID != nil {
		n.SetTraceID(*traceID)
	}

	if err := s.notificationRepo.Create(ctx, n); err != nil {
		return nil, fmt.Errorf("failed to create escalation notification: %w", err)
	}

	s.logger.Info().
		Str("notification_id", n.NotificationID.String()).
		Str("action_id", n.ActionID.String()).
		Str("channel", string(n.Channel)).
		Msg("escalation notification created")
