...
import (
	...
	actionMocks "github.com/execution-hub/execution-hub/internal/domain/action/mocks"
	"github.com/execution-hub/execution-hub/internal/domain/notification"
	notificationMocks "github.com/execution-hub/execution-hub/internal/domain/notification/mocks"
	"github.com/execution-hub/execution-hub/internal/domain/rule"
	ruleMocks "github.com/execution-hub/execution-hub/internal/domain/rule/mocks"
)
...
func TestService_CreateFromAction(t *testing.T) {
	t.Run("success with defaults", func(t *testing.T) {
		...
		notificationRepo.EXPECT().
			GetByActionID(ctx, action.ActionID).
			Return(nil, nil)
	...
	t.Run("dedupe by key", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		...
		ctx := context.Background()
		actionConfig := json.RawMessage(`{"userId": "user123"}`)
		action := domainAction.NewAction(uuid.New(), 1, uuid.New(), domainAction.TypeNotify, actionConfig)
		action.SetDedupeKey("source_device")
		action.CreatedAt = time.Now().UTC().Add(-2 * time.Minute)
		action.SetCooldown(action.CreatedAt.Add(10 * time.Minute))

		existing := notification.NewNotification(action.ActionID, notification.ChannelSSE, notification.PriorityMedium, "Title", "Body", nil)
		...
		notificationRepo.EXPECT().
			FindByDedupeKey(ctx, "source_device|channel:SSE|user:user123", gomock.Any()).
			Return(existing, nil)
