...
func (m *MockRepository) InsertEvent(ctx context.Context, event *trust.EventEvidence) error {
	args := m.Called(ctx, event)
	return args.Error(0)
}

func (m *MockRepository) GetEvent(ctx context.Context, eventID uuid.UUID) (*trust.EventEvidence, error) {
	args := m.Called(ctx, eventID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*trust.EventEvidence), args.Error(1)
}

func (m *MockRepository) GetEvents(ctx context.Context, eventIDs []uuid.UUID) ([]trust.EventEvidence, error) {
...
func TestService_AddToHashChain(t *testing.T) {
	...
	t.Run("adds first entry to chain (genesis)", func(t *testing.T) {
		...
		mockRepo.On("InsertEvent", ctx, mock.AnythingOfType("*trust.EventEvidence")).Return(nil)
